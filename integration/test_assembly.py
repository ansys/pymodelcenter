...
def test_correctly_gets_child_elements_proc_haschildren(process_sequence_workflow):
    ...
    child_elements = assembly.get_elements()
    child_elements_as_list = [element for element in child_elements.values()]
...
def test_correctly_gets_child_elements_proc_after_move(process_sequence_workflow):
    # Setup
    assembly: mc_api.IAssembly = process_sequence_workflow.get_assembly(
        "Model.main_branch.has_items"
    )

    comp_to_move: mc_api.IComponent = process_sequence_workflow.get_component(
        "Model.main_branch.has_items.Script"
    )
    process_sequence_workflow.move_component(comp_to_move, assembly, 1)
