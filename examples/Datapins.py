...
with grpcmc.Engine() as mc:
    ...
    with mc.load_workflow(workflow_path) as workflow:
        ...
        # 1) is_valid: it indicates if the value has been calculated in MC using the most recent input values
        # 2) value
        # 3) safe_value
        # The difference between 2) and 3) is that if you try to access safe_value before the workflow has been run/updated, 
        # you will raise a VariableValueInvalidError. The value property will always return the current
        # value regardless whether it is valid or not.
