"""Classes for representing variables that exist in ModelCenter with unsupported value types."""

import ansys.common.variableinterop as acvi
import grpc
from overrides import overrides

from .proto.element_messages_pb2 import ElementId
from .variable import BaseVariable


class VariableWithUnsupportedTypeException(BaseException):
    """This exception is raised when attempting to interact with a variable of unsupported type."""

    def __init__(self):
        """Create a new instance."""
        super(VariableWithUnsupportedTypeException, self).__init__(
            "The pyModelCenter API does not currently support this interaction on variables of "
            "this type."
        )


class UnsupportedTypeVariable(BaseVariable):
    """
    Represents a variable with an unsupported datatype.

    Generally speaking, it will be possible to perform interactions that don't require
    retrieving or otherwise interacting with the variable's value or metadata. Attempts to
    set or get the value or metadata will raise a VariableWithUnsupportedTypeException.
    """

    def __init__(self, element_id: ElementId, channel: grpc.Channel):
        """Construct a new instance."""
        super(BaseVariable, self).__init__(element_id=element_id, channel=channel)

    @overrides
    def set_metadata(self, new_metadata: acvi.CommonVariableMetadata) -> None:
        raise VariableWithUnsupportedTypeException()

    @overrides
    def set_value(self, value: acvi.VariableState) -> None:
        raise VariableWithUnsupportedTypeException()

    @overrides
    def get_metadata(self) -> acvi.CommonVariableMetadata:
        raise VariableWithUnsupportedTypeException()
